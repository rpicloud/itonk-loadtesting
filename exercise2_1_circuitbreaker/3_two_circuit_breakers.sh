...
if [ -z "$HOST" ]
  then
	...
	echo
	echo 
	echo SETUP DONE - Two circuitbreakers, no fallback, $DELAYHOST2 ms delayhost2, $RESULTS results, $RATE rate, $DURATION duration

	echo RUNNING TEST

	NOW=$(date "+%m-%d_%H-%M-%S")
	DIR=3_circuit_breaker_h${HOST}_r${RESULTS}_d${DELAYHOST2}_rate${RATE}_${NOW}
	mkdir $DIR
