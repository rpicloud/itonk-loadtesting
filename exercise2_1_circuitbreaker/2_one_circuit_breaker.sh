...
if [ -z "$HOST" ]
  then
	...
	NOW=$(date "+%m-%d_%H-%M-%S")
	DIR=2_one_circuit_breaker_h${HOST}_r${RESULTS}_d${DELAYHOST2}_rate${RATE}_${NOW}
	mkdir $DIR
	echo "GET http://"$HOST0"/resources_onecb" | ../vegeta attack -duration=${DURATION} -rate=$RATE | tee $DIR/results.bin | ../vegeta report
